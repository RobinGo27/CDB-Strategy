...
def recursive_train(X, y, Z, train_window):
    ...
    for t in range(train_window, X.shape[0]):
        ...
        X_train = (X_train.T.reshape(-1, 1)).T
        X_test = X[t]
        # X_test = (X_test.T/np.std(X_test, axis = 0).reshape(-1, 1).flatten()).T
        X_test = (X_test.T.reshape(-1, 1).flatten()).T
        y_train = y[t-52:t]
...
# X = factors_df.drop('中债商业银行同业存单到期收益率(AAA):1年:-中期借贷便利(MLF):利率:1年:周', 1)
# X = factors_df.iloc[:, 4:8]
X = factors_df.drop('中债商业银行同业存单到期收益率(AAA):1年:-中期借贷便利(MLF):利率:1年:周', 1)
X = LLT(X, 12)
...
for t in range(52, X.shape[0]):
    Z = autoproxy(X[t-52:t], y[t-52:t], 1)
    X_train = X[t-52:t]
